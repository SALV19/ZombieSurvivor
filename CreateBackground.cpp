...
int createBackground(VertexArray& rVA, IntRect arena)
{
	...
	rVA.resize(worldHeight * worldHeight * VERTS_IN_QUAD);
	...
	for (int w = 0; w < worldWidth; w++)
	{
		for (int h = 0; h < worldHeight; h++)
		{
			...
			//Define the position in the Textures
			...
			||	w == 0	|| w == worldWidth -1)
			{
				...
				rVA[currentVertex + 2].texCoords =
					Vector2f(TILE_SIZE, TILE_SIZE + TILE_TYPE * TILE_SIZE);
				rVA[currentVertex + 3].texCoords =
					Vector2f(0, TILE_SIZE + TILE_TYPE * TILE_SIZE);
